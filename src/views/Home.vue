<template>
  <div class="w-full">
    ...
    <portal to="modals">
      <rule-modal v-if="modal === 'shark'" @close="modal = null">
        <template v-slot:title>Shark</template>
        <template v-slot:content>
          <p class="py-2">The main element of the game is the projected <strong>shark</strong> on the water surface. The <strong>shark</strong> is projected to approach ('swim') towards the player holding the ball. The throwing player must, therefore, make sure to pass the ball to one of the receiving players before the <strong>shark</strong> can reach them. If a player successfully throws the ball to another team player, the <strong>shark</strong> will flip its swimming direction to move directly towards its 'new' target. If the player throws the ball without reaching one of their team members, they will have to retrieve it before the <strong>shark</strong> gets the ball.</p>
          <p class="py-2">To motivate children to go after the ball, when a player becomes the throwing player a circle will be projected around them to show that they are in danger of being caught by the <strong>shark</strong>. Contrary to the general <strong>"Keep Away"</strong> game where one player is in the middle trying to intercept, <strong>"Sharkball"</strong> is a player versus player game, meaning that there is no motivation for a receiving player to retrieve a ball thrown incorrectly by the throwing player. In other words, the game must always appoint someone as the throwing player who stands to lose a life unless the ball is passed to the next person.</p>
        </template>
      </rule-modal>
      <rule-modal v-if="modal === 'octopus'" @close="modal = null">
        <template v-slot:title>Octopus</template>
        <template v-slot:content>
          <p class="py-2">One of the components shows an <strong>octopus</strong> projected around a receiving player, meaning that they are effectively <strong>blocked</strong> from receiving the ball by the throwing player (much like a defending opponent would block a team member). The throwing player must, therefore, quickly assess which of their team members is able to receive the ball.</p>
        </template>
      </rule-modal>
      <rule-modal v-if="modal === 'jellyfish'" @close="modal = null">
        <template v-slot:title>Jellyfish</template>
        <template v-slot:content>
          <p class="py-2">Another component included are the <strong>jellyfishes</strong> that will be projected at random spots on the water surface. If the ball is thrown into an area occupied by a <strong>jellyfish</strong>, the throwing player will lose a life. This provides environmental constraints to the throwing player, letting them decide between easier and riskier passes.</p>
        </template>
      </rule-modal>
      <rule-modal v-if="modal === 'otters'" @close="modal = null">
        <template v-slot:title>Sea Otters</template>
        <template v-slot:content>
          <p class="py-2">Besides blocking sea creatures, the game may project randomly <strong>sea otters</strong> as distractions for the players. These projections won’t affect the players’ lives, thus requiring the throwing player to quickly assess whether the visualisations are important to the gameplay and - if needed - adapt the passing strategy to them.</p>
        </template>
      </rule-modal>
      <rule-modal v-if="modal === 'submarine'" @close="modal = null">
        <template v-slot:title>Submarine</template>
        <template v-slot:content>
          <p class="py-2">The last component included in the game are <strong>submarines</strong>, which will appear randomly inside of the circle. If a player successfully hits a <strong>submarine</strong> with the ball, they will receive an additional life. If a player runs out of lives, the <strong>submarine</strong> will appear outside of the playing circle. The player must then swim to it and “catch” it to restore their lives and continue with the normal game.</p>
        </template>
      </rule-modal>
    </portal>
  </div>
